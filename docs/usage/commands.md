...
1. **`init`**: Typically, you start a project with `bash2yaml init` to create the configuration and directory
   structure.
2. **`compile`**: You then run `bash2yaml compile` to generate the final CI/CD files from your source.
3. **`lint`**: After compiling, you can use `bash2yaml lint` to validate the generated YAML files (via API for
