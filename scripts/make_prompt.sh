...
# readonly SCRIPT_NS="bash2yaml_proc"
readonly SRC_DIR="./bash2yaml"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/bash2yaml"
readonly OUTPUT_MD_BASENAME="bash2yaml.flat.md"

# Copy directory safely
bash2yaml_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/bash2yaml
bash2yaml_proc::run_strip_docs() {
  local dir
...
# in the generated Markdown (bash2yaml.flat.md). Other code fences remain untouched.
bash2yaml_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  bash2yaml_proc::copy_to_tmp
  bash2yaml_proc::run_strip_docs
  bash2yaml_proc::confirm "Run coderoller-flatten-repo?"
  bash2yaml_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  bash2yaml_proc::clean_flat_md_trailing_double_hash
  bash2yaml_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  bash2yaml_proc::cleanup_tmp
}
