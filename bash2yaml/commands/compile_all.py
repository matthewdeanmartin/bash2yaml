...
from bash2yaml.commands.clean_all import report_targets
...
from bash2yaml.commands.compile_not_bash import maybe_inline_interpreter_command
from bash2yaml.commands.hash_path_helpers import find_hash_file, get_output_hash_path, migrate_hash_file
...
def infer_cli(
    ...
    command = f"bash2yaml compile --in {short_path(input_dir)} --out {short_path(output_path)}"
    if dry_run:
        command += " --dry-run"
    if parallelism:
