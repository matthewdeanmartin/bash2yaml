...
def check_precommit_hook_status(repo_root: Path) -> tuple[PrecommitStatus, list[str]]:
    """
    Checks the status of the bash2yaml pre-commit hook.
    """
    try:
        ...
        return "Foreign Hook", [
            f"A non-bash2yaml hook exists at: {short_path(h_path)}",
...
def check_map_source_paths_exist() -> list[str]:
    """
    Verifies that all source directories specified in the [tool.bash2yaml.map]
    configuration exist.
    """
    warnings = []
...
def check_lint_config_validity(cfg: Config) -> list[str]:
    ...
    headers = {
        "User-Agent": "bash2yaml-doctor/1.0",
...
#
#     api_url = f"{gitlab_url.rstrip('/')}/api/v4/projects/{project_id}"
#     headers = {"User-Agent": "bash2yaml-doctor/1.0"}
#     if token:
#         headers["PRIVATE-TOKEN"] = token
#
