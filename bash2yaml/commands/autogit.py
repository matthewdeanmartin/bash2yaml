...
def _run_git_command(command: list[str], cwd: Path) -> str:
    ...
    try:
        ...
        raise Bash2YamlError("Git command not found. Is Git installed and in your PATH?") from None
    except subprocess.CalledProcessError as e:
        logger.error("Git command failed: git %s", " ".join(command))
        logger.error("Stderr:\n%s", e.stderr)
        raise Bash2YamlError(f"Git command failed. Stderr: {e.stderr.strip()}") from e
...
def _paths_under_repo(repo_root: Path, *paths: Path) -> list[Path]:
    ...
    Raises Bash2YamlError if any provided path is outside the repository.
    """
    kept: list[Path] = []
    for p in paths:
        try:
            ...
            raise Bash2YamlError(f"Configured path {p} is not inside repository root {repo_root}.") from ve
...
def run_autogit(config: Config, commit_message: str | None = None) -> int:
    ...
    try:
        ...
        if mode in ["commit", "push"]:
            message = commit_message or config.autogit_commit_message or "chore: auto-commit by bash2yaml"
        ...
        if mode == "push":
            ...
            if not branch:
                ...
                if branch == "HEAD":
                    raise Bash2YamlError("Detached HEAD; set [autogit].branch or check out a branch before pushing.")
