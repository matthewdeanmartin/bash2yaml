...
def create_or_update_config_file(base_path: Path, config_data: dict[str, Any], force: bool = False):
    ...
    # Create/replace the [tool.bash2yaml] table
    ...
    tool_table["bash2yaml"] = b2gl_table  # type: ignore[index]

    # Add comments for clarity
    tool_table["bash2yaml"].comment("Configuration for bash2yaml")  # type: ignore[union-attr,index]
    if "input_dir" in tool_table["bash2yaml"]:  # type: ignore[operator,index]
        tool_table["bash2yaml"].item("input_dir").comment("Directory for source .yml and .sh files")  # type: ignore[union-attr,index]
    if "output_dir" in tool_table["bash2yaml"]:  # type: ignore[union-attr,index,operator]
        tool_table["bash2yaml"].item("output_dir").comment("Directory for compiled GitLab CI files")  # type: ignore[union-attr,index]
...
def run_init(directory, force) -> int:
    ...
    console = Console()
    console.print("\n[bold]Initializing a new bash2yaml project...[/bold]\n")
    ...
    try:
        ...
        input_dir = Path(base_path / user_config["tool"]["bash2yaml"]["input_dir"])
