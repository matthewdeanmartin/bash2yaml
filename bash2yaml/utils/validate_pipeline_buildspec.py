"""AWS CodeBuild Buildspec validation against SchemaStore JSON schema.

AWS CodeBuild has no official lint API — validation is schema-only.
Schema source: ``https://json.schemastore.org/buildspec.json``
"""
...
class BuildspecValidator:
    ...
    def _fetch_schema_from_url(self) -> dict[str, Any] | None:
        try:
            ...
            logger.warning("Failed to fetch AWS CodeBuild Buildspec schema from URL: %s", e)
            return None
    ...
    def _load_fallback_schema(self) -> dict[str, Any] | None:
        try:
            ...
            logger.warning("Failed to load AWS CodeBuild Buildspec schema from package resource: %s", e)
    ...
    def validate_buildspec(self, yaml_content: str) -> tuple[bool, list[str]]:
        """Validate AWS CodeBuild Buildspec YAML against the schema.
