"""Semaphore CI pipeline validation against JSON schema.

Semaphore has no official lint API — validation is schema-only.
Schema source: ``https://raw.githubusercontent.com/semaphoreci/spec/main/schemas/pipeline.json``
"""
...
class SemaphoreValidator:
    """Validates Semaphore CI YAML files against the JSON schema."""

    def __init__(self, cache_dir: str | None = None):
        self.schema_url = "https://raw.githubusercontent.com/semaphoreci/spec/main/schemas/pipeline.json"
        self.cache_dir = Path(cache_dir) if cache_dir else Path(tempfile.gettempdir())
        self.cache_file = self.cache_dir / "semaphore_schema.json"
        self.fallback_schema_path = "schemas/semaphore_schema.json"
    ...
    def _fetch_schema_from_url(self) -> dict[str, Any] | None:
        try:
            ...
            logger.warning("Failed to fetch Semaphore CI schema from URL: %s", e)
            return None
    ...
    def _load_fallback_schema(self) -> dict[str, Any] | None:
        try:
            ...
            logger.warning("Failed to load Semaphore CI schema from package resource: %s", e)

        return None

    @cache  # noqa: B019
    def get_schema(self) -> dict[str, Any]:
        """Get the Semaphore CI schema: cache -> URL -> fallback."""
        schema = self._load_schema_from_cache()
        if schema:
            logger.debug("Using cached Semaphore CI schema")
            return schema
        ...
        if schema:
            logger.debug("Using Semaphore CI schema from package")
            return schema

        raise RuntimeError("Could not load Semaphore CI schema from URL, cache, or fallback resource")

    def validate_pipeline(self, yaml_content: str) -> tuple[bool, list[str]]:
        """Validate Semaphore CI YAML against the schema.
