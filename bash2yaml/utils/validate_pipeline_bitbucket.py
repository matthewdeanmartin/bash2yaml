"""Bitbucket Pipelines validation against SchemaStore JSON schema.

Bitbucket has no official lint API — validation is schema-only.
Schema source: ``https://json.schemastore.org/bitbucket-pipelines.json``
"""
...
class BitbucketValidator:
    """Validates Bitbucket Pipelines YAML files against the SchemaStore schema."""

    def __init__(self, cache_dir: str | None = None):
        self.schema_url = "https://json.schemastore.org/bitbucket-pipelines.json"
        self.cache_dir = Path(cache_dir) if cache_dir else Path(tempfile.gettempdir())
        self.cache_file = self.cache_dir / "bitbucket_pipelines_schema.json"
        self.fallback_schema_path = "schemas/bitbucket_pipelines_schema.json"
    ...
    def _fetch_schema_from_url(self) -> dict[str, Any] | None:
        try:
            ...
            logger.warning("Failed to fetch Bitbucket Pipelines schema from URL: %s", e)
            return None
    ...
    def _load_fallback_schema(self) -> dict[str, Any] | None:
        try:
            ...
            logger.warning("Failed to load Bitbucket Pipelines schema from package resource: %s", e)

        return None

    def get_schema(self) -> dict[str, Any]:
        """Get the Bitbucket Pipelines schema: cache -> URL -> fallback."""
        if self._schema is not None:
            return self._schema

        schema = self._load_schema_from_cache()
        if schema:
            logger.debug("Using cached Bitbucket Pipelines schema")
            self._schema = schema
            return schema
        ...
        if schema:
            logger.debug("Using Bitbucket Pipelines schema from package")
            self._schema = schema
            return schema

        raise RuntimeError("Could not load Bitbucket Pipelines schema from URL, cache, or fallback resource")

    def validate_pipeline(self, yaml_content: str) -> tuple[bool, list[str]]:
        """Validate Bitbucket Pipelines YAML against the schema.
