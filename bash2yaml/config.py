...
try:
    import bash2yaml.utils.toml_reader as _toml
    from bash2yaml.utils.toml_reader import read_toml  # type: ignore
...
class Config:
    """
    Manages configuration for bash2yaml, loading from files and environment variables.

    Configuration is loaded with the following priority:
    1. Environment variables (e.g., BASH2YAML_LINT_GITLAB_URL)
    ...
    ENV_VAR_PREFIX = "BASH2YAML_"
    CONFIG_FILES = [".bash2yaml.toml", "bash2yaml.toml", "pyproject.toml"]
    ...
    def load_file_config(self) -> dict[str, Any]:
        """Loads configuration from bash2yaml.toml or pyproject.toml using toml_reader."""
        ...
        try:
            ...
            if config_path.name == "pyproject.toml":
                file_config = data.get("tool", {}).get("bash2yaml", {})  # type: ignore[assignment]
            else:
    ...
    def load_env_config(self) -> dict[str, str]:
        ...
        for key, value in os.environ.items():
            if key.startswith(self.ENV_VAR_PREFIX):
                # Converts BASH2YAML_SECTION_KEY to section_key
